<template>
    <div class="z-10 fixed inset-0 flex flex-col items-center justify-end space-y-4 bg-black bg-opacity-50">
        ...
        <form @submit.prevent="submit" class="self-stretch bg-white rounded-t-xl px-6 py-4 space-y-4">
            ...
            <hr class="-mx-6">
            <div class="space-y-1">
                <label class=" font-medium text-gray-700">Measurements</label>
            ...
            <div class="space-y-1">
                <label class=" font-medium text-gray-700">Ingredient</label>
                <input v-model="form.name" type="text" class="px-3 py-2 block w-full rounded-lg border border-gray-200 bg-gray-200 focus:bg-white
                    focus:outline-none focus:shadow-outline-indigo focus:border-indigo-300 transition-shadow duration-100 ease-linear">
            </div>
            <button type="submit" class="rounded-full px-4 py-3 font-medium block w-full bg-pink-500 text-white focus:outline-none" :class="{ 'opacity-50 cursor-not-allowed': ! isValid }" :disabled="! isValid">Add ingredient</button>
        </form>
    </div>
</template>
